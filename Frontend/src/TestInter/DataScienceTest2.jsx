...
export default function DataScienceTest2() {
  ...
  const startNewTest = () => {
    ...
    while (picked.length < 20) {
      const i = Math.floor(Math.random() * INTER_QUESTIONS.length);
      if (!usedIndexes.has(i)) {
        picked.push(INTER_QUESTIONS[i]);
        usedIndexes.add(i);
      }
    }
                ...
                <button
                  onClick={() =>
                    navigate("/certificate", {
                      state: { type: "datascience", level : "intermediate" },
                    })
                  }
